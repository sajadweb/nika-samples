# Nika
## Project Overview
Nika is a small web service scaffold written in Go, intended as a lightweight starter for building APIs and microservices.

## Framework
Nika uses the Gin web framework for routing and middleware and relies on Go modules for dependency management.

## Requirements
- Go 1.20 or newer
- Git

## Local Installation & Run
1. Clone the repository:
    `git clone <repository-url>`
2. Enter the project directory:
    `cd nika-app`
3. Download dependencies:
    `go mod download`
4. Run for development: 
     `go install github.com/air-verse/air@latest`
     `air` 

## Environment Configuration
Configuration is read from environment variables. Common examples:
- `PORT` — service port (default: 3007)


## Tests
Run tests with:
`go test ./...`

## Quick Usage
Start example:
`curl http://localhost:3007/`
`curl http://localhost:3007/sajjad`


 
### POST Request (Create)

```javascript
async function postNika() {
  try {
    const response = await fetch('http://localhost:3006/nika', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        name: 'Nika',
        description: 'Creating a new Nika entry'
      }),
    });

    // Check if the response is successful (status 200-299)
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }

    const data = await response.json();
    console.log('POST Success:', data);
  } catch (error) {
    console.error('POST Error:', error);
  }
}

// Call the function
postNika();
```

### PUT Request (Update)

```javascript
async function putNika() {
  try {
    const response = await fetch('http://localhost:3007/nika', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        id: 1, // Usually required for PUT to know WHAT to update
        name: 'Nika Updated',
        description: 'Updating an existing Nika entry'
      }),
    });

    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }

    const data = await response.json();
    console.log('PUT Success:', data);
  } catch (error) {
    console.error('PUT Error:', error);
  }
}

// Call the function
putNika();
```

## License
See the LICENSE file for licensing details.
